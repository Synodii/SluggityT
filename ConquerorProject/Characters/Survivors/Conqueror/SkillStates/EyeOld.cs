...
namespace ConquerorMod.Survivors.Conqueror.SkillStates
{
    public class Advance : BaseSkillState
    {
        ...
        private GameObject healeyeboom = Addressables.LoadAssetAsync<GameObject>("RoR2/Base/BleedOnHitAndExplode/BleedOnHitAndExplodeDelay.prefab").WaitForCompletion();
        private GameObject dmgeyeboom = Addressables.LoadAssetAsync<GameObject>("RoR2/Base/BleedOnHitAndExplode/BleedOnHitAndExplode_Explosion.prefab").WaitForCompletion();
        private GameObject dmginiteyeboom = Addressables.LoadAssetAsync<GameObject>("RoR2/Base/ImpBoss/ImpVoidspikeExplosion.prefab").WaitForCompletion();
        private GameObject dmginiteyeboom2 = Addressables.LoadAssetAsync<GameObject>("RoR2/Base/BleedOnHitAndExplode/BleedOnHitAndExplode_Impact.prefab").WaitForCompletion();
        ...
        private IEnumerator ExplodeEnemyAfterDelay(CharacterBody nmebody, float delay)
        {
            ...
            CreateDamageEyeEffect(Util.GetCorePosition(nmebody.gameObject));
        ...
        private void CreateHealEyeEffect(Vector3 origin)
        {
            EffectData effectData = new EffectData();
            //effectData.rotation = Util.QuaternionSafeLookRotation(forwardDirection);
            effectData.origin = origin;
            effectData.scale = 4f;
            EffectManager.SpawnEffect(healeyeboom, effectData, false);
        }

        private void CreateInitialDamageEyeEffect(Vector3 origin)
        {
            EffectData effectData = new EffectData();
            //effectData.rotation = Util.QuaternionSafeLookRotation(forwardDirection);
            effectData.origin = origin;
            effectData.scale = 13f;
            EffectManager.SpawnEffect(dmginiteyeboom, effectData, false);
        }

        private void CreateDamageEyeEffect(Vector3 origin)
        {
            EffectData effectData = new EffectData();
            //effectData.rotation = Util.QuaternionSafeLookRotation(forwardDirection);
            effectData.origin = origin;
            effectData.scale = 5f;
            EffectManager.SpawnEffect(dmgeyeboom, effectData, false);
        }

        private void CreateDamageEyeEffect2(Vector3 origin)
        {
            EffectData effectData = new EffectData();
            //effectData.rotation = Util.QuaternionSafeLookRotation(forwardDirection);
            effectData.origin = origin;
            effectData.scale = 6.5f;
            EffectManager.SpawnEffect(dmginiteyeboom2, effectData, false);
        }
        ...
        private void Fire()
        {
            if (NetworkServer.active)
            {
                if (characterBody.GetComponent<ConquerorController>().bagDeployed == false)
                {
                    ...
                    CreateHealEyeEffect(Util.GetCorePosition(base.gameObject));
                    //characterBody.AddTimedBuff(ConquerorBuffs.satiatedBuff, 5f);
                ...
                else
                {
                    ...
                    CreateInitialDamageEyeEffect(Util.GetCorePosition(base.gameObject));
                    ...
                    for (int i = 0; i < targetsHit.hitCount; i++)
                    {
                        ...
                        CreateDamageEyeEffect2(Util.GetCorePosition(nmebody.gameObject));
